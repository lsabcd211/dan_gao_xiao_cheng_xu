...
<script>
	import * as allApi from '@/api/index.js'
	export default {
		data() {
			return {
				banners:[],
		...
		onShow() {
			...
			// });
			// // #endif
			allApi.goodList().then(e=>{
				console.log(e)
				this.banners=this.banners.concat(e.banners);
				this.goods=this.goods.concat(e.goods);
				this.icons=this.icons.concat(e.icons);
				this.$store.state.goods=this.goods;
			})
		},
		methods: {
